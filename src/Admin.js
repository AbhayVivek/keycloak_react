...
const Admin = () => {
	...
	const [FormState, setFormState] = useState("addnew");
	...
	const fetchUsers = async () => {
		try {
			const response = await axios.get("http://10.3.0.16/api/users");
			setUsers(response.data);
		} catch (error) {
			console.error("Error fetching users", error);
		}
	...
	const fetchRoles = async (id) => {
		try {
			const response = await axios.get(
				`http://10.3.0.16/api/users/${id}/roles/realm`
			);
			setRoles(response.data);
		} catch (error) {
			console.error("Error fetching roles", error);
	...
	const handleUserClick = async (user) => {
		try {
			const response = await axios.get(
				`http://10.3.0.16/api/users/${user.id}/attributes`
			);
			const userDetails = { ...user, attributes: response.data };
			setSelectedUser(userDetails);
			fetchRoles(user.id);
	...
	const handleAddUser = async () => {
		...
		try {
			const response = await axios.post("http://10.3.0.16/api/users", newUser);
			setUsers([...users, response.data]);
			setFormState("addnew");
		} catch (error) {
			console.error("Error adding user", error);
	...
	const handleUpdateUser = async (userData) => {
		try {
			await axios.put(`http://10.3.0.16/api/users/${userData.id}`, userData);
			await axios.put(`http://10.3.0.16/api/users/${userData.id}/attributes`, {
				attributes: userData.attributes,
			});
			fetchUsers();
		} catch (error) {
	...
	const handleAddRole = async (roleName) => {
		try {
			const response = await axios.post(
				`http://10.3.0.16/api/users/${selectedUser.id}/roles/realm`,
				{
					roleName: roleName,
				}
			);
